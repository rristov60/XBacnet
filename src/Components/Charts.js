...
const Charts = ({ subscriptionToPlot, subscriptions, activeSubscriptions }) => {
  ...
  const createRows = () => {
    const theSubscription = activeSubscriptions.filter(
      (x) =>
        x.device == subscriptionToPlot.device &&
        x.type == subscriptionToPlot.objectType &&
        x.instance == subscriptionToPlot.instance
    );

    if (theSubscription.length != 0) {
      var currentSubscription = subscriptions.filter(
        (x) =>
          x.device == theSubscription[0].device &&
          x.type == theSubscription[0].type &&
          x.instance == theSubscription[0].instance
      );

      if (
        currentSubscription.length != 0 &&
        currentSubscription[0].values != undefined
      ) {
        ...
        <ResponsiveContainer width="95%" height="93%">
          <LineChart margin={{ top: 5, right: 10, bottom: 5, left: 10 }}>
            ...
            <XAxis
              dataKey="time"
              stroke="white"
              tick={{ fontSize: "0.8rem", fontFamily: "League Spartan" }}
            />
            <YAxis
              stroke="white"
              tick={{ fontSize: "0.8rem", fontFamily: "League Spartan" }}
            />
            {
              <>
                <Line
                  data={rows}
                  type="monotone"
                  dataKey={subscriptionToPlot.OBJECT_NAME}
                  stroke="#A3E635"
                />
              </>
            }
