  ...
  const subscribeCOV = () => {
    ...
    window.bacnet.subscribeCOV(device, subscribeObject, (response) => {
      if (response == undefined) {
        ...
        setToastMessage(
          `Successfully subscribed to COV for: ${variable.OBJECT_NAME.value}!`
        );
    ...
    window.bacnet.unsubscribeCOV(device, unsubscribeObject, (response) => {
      if (response == undefined) {
        ...
        var unsubscribeVar = {
          ...
          values: [variable.PRESENT_VALUE],
        };

        setToastMessage(
          `Successfully unsubscribed to COV for: ${variable.OBJECT_NAME.value}!`
        );
        setToastType("success");
        setToastOpen(true);
        setTimeout(() => {
  ...
  return (
    <>
      {Object.keys(variable).length === 0 ? (
        ...
        <>
          {variable?.cov?.subscribed == false ||
          variable?.cov?.subscribed == undefined ? (
            // If there isn't a scan running in the momment
            <>
              <Tooltip
                title="Subscribe to COV for the selected object"
                TransitionComponent={Zoom}
                arrow
              >
                <Button
                  onClick={subscribeCOV}
                  variant="outlined"
                  style={{ borderColor: "#A3E635", color: "#A3E635" }}
                >
                  Subscribe COV
                </Button>
              </Tooltip>
            ...
            <>
              <Tooltip
                title="Unsubscribe to COV for the selected object"
                TransitionComponent={Zoom}
                sx={{ backgroundColor: "transparent" }}
                arrow
              >
                <Button
                  onClick={unSubscribeCOV}
                  variant="outlined"
