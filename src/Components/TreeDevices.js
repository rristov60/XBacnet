    ...
    if (device.variables == undefined) {
      ...
      const intialReadRequestArray = [
        {
          ...
          properties: [
            ...
            {
              // Read all properties for device object
              id: 8,
            },
          ],
        },
      ...
      window.bacnet.readMultiple(device, intialReadRequestArray, (response) => {
        if (!response.error) {
          ...
          setToastMessage(
            `Successfully read all variables for #${device.deviceId}!`
          );
      ...
      {devices.length > 0 ? (
        ...
        <>
          ...
          >
            <StyledTreeItem
              nodeId="1"
              label={<span style={{ fontSize: "0.9rem" }}>Devices</span>}
                  ...
                  <StyledTreeItem
                    ...
                    label={
                      <span style={{ fontSize: "0.8rem" }}>
                        {`[ #${device.deviceId} ] ${
                          device.info?.OBJECT_NAME?.value == undefined
                            ? device.address
                            : `${device.info.OBJECT_NAME.value} (${device.info.LOCATION.value})`
                        }`}
                      </span>
                    }
                  />
