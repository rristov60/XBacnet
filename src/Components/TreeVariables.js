...
const divideIpPort = (string, part) => {
  ...
  if (part == "Ip") return formatted[0];
  else return formatted[1];
};
      ...
      window.bacnet.readMultiple(device, requestArray, (response) => {
        ...
        if (!response.error) {
          response.value.values[0].values.map((value) => {
            Object.keys(bacnetProperties).map((key) => {
              // Getting the name of the variable type
          ...
          setToastMessage(
            `Successfully read object: ${variable.OBJECT_NAME.value}!`
          );
          ...
          if (theResponse.includes("BacnetAbort")) {
            ...
            setToastMessage(
              `${errorsDescription.AbortReason[`${abortReason}`]}!`
            );
          } else {
            ...
            if (errorsDescription.ErrorCodes[responseFormatted] != undefined)
              setToastMessage(
                `${errorsDescription.ErrorCodes[responseFormatted]}!`
              );
            else setToastMessage(`${response.error}`);
          }
        ...
        <>
          ...
          >
            <StyledTreeItem
              ...
              onClick={() => {
                {
                  updateSelectedVar(device, device.info);
                }
              }}
              label={
                <span
                  style={{ fontSize: "0.9rem" }}
                >{`${device.info.OBJECT_NAME.value} \n [ IP: ${device.address}, #${device.deviceId} ]`}</span>
              }
            >
              ...
              {device.variables.map((variable) => {
                return (
                  <StyledTreeItem
                    onClick={() => {
                      updateSelectedVar(device, variable);
                      {
                        /* HANDLE THE READING OF THE NAME HERE AND DISPLAYING THE VARS */
                      }
                    }}
                    ...
                    label={
                      <span style={{ fontSize: "0.78rem" }}>
                        {`${
                          variable.OBJECT_NAME == undefined
                            ? `${variable.typeName} ( Instance: ${variable.value.instance} )`
                            : `${variable.OBJECT_NAME.value} (${variable.typeName})`
                        } `}
                      </span>
                    }
                  />
