    ...
    if (valueToWrite != "UNIQUEERRSTRINGNOTTOWRITE") {
      ...
      window.bacnet.writeToObject(device, writeObject, (response) => {
        if (response.error == null) {
          ...
          // update the variable
          setToastMessage(
            `Successfully written ${writeObject.theValue[0].value} to ${variable.OBJECT_NAME.value}!`
          );
      ...
      Object.keys(variable).map((key) => {
        ...
        if (
          ...
          rows.push({
            ...
            value:
              typeof variable[key].value === "object" &&
      ...
      {variable.DESCRIPTION == undefined ? (
        ...
        <>
          <AlertDialog
            ...
            setValueToWrite={setValueToWrite}
            setObjectToWrite={setObjectToWrite}
...
const columns = [
  {
    field: "BACnetProperty",
    headerName: "BACnet Property",
    width: 190,
    editable: false,
    headerAlign: "center",
